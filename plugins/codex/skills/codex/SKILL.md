...
**Spawn an EXTRA pane/window ONLY when the user explicitly asks for a genuinely separate or parallel codex** — phrases like "in parallel", "a second/separate codex", "keep this one running and also start another", "don't interrupt the current task", "spin up another". In that case (and only then) use the topic-naming protocol + `new` (see below). For everything else, the single reused pane/window is the answer.

> **"Side panel" / "side by side" / "beside me" / "next to me" / "in a pane" is the DEFAULT — NOT an extra window.** Those phrases describe the in-current-window pane that `pane` already produces (codex sits right next to Claude in the same window). Resolve `$TARGET` normally (→ `pane`); do **not** interpret them as a request for a separate `cc-codex` window via `new`. Only the genuinely-parallel phrasing above warrants `new`.
