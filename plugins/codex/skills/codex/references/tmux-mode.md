...
- **When codex exits.** Panes use `remain-on-exit failed`: when codex exits **cleanly** (status 0 — it finished, or the user closed it), the pane **auto-closes** so no dead "Pane is dead (status 0)" corpse lingers in the window. Only a **crash** (non-zero exit) keeps the pane as `dead` so you can read the error. Override with `CC_CODEX_REMAIN_ON_EXIT=on` (always keep, old behavior) or `off` (always close).
